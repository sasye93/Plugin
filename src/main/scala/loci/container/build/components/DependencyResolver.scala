/**
  * Checks validity of dependencies and macro usages, and filters the information pickled by macros.
...
class DependencyResolver(implicit val plugin : Containerize) {
  ...
  def dependencies(entryPoints : TEntryList, peerDefs : TPeerList) : (TEntryList, TPeerList) = {
    ((filterInvalid _).tupled andThen (filterInvalidPeerRefs _).tupled)(entryPoints, peerDefs)
  }
}
