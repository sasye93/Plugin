/**
  * Config class, the common superclass of container {@link loci.container.impl.types.ContainerConfig} and module {@link loci.container.impl.types.ModuleConfig} configs.
...
abstract class Config(json : Option[String], homeDir : Option[String] = None)(implicit io : IO, implicit private val plugin : Containerize) {
  ...
  protected def loadConfig() : String = {
    ...
    if (json.isDefined){
      ...
      ServiceConfig.defaultContainerConfig.JSON
    }
  }
...

  {
    ...
    if(cfgType.isDefined){
      ...
      if(cfgType.get == "service" && (cfgClass != Options.toolbox.toInstanceClassName(ServiceConfig))) io.logger.error("You cannot pass a service config as a module config (type=service).")
    }
  }
