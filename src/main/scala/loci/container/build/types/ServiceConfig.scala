...
class ServiceConfig(json : Option[String], moduleConfig : Option[ModuleConfig])(implicit io : IO, implicit private val plugin : Containerize) extends Config(json, {
  if(moduleConfig.isDefined) moduleConfig.get.getHome else None
})(io, plugin) {

  import ServiceConfig.{defaultContainerConfig => default}
