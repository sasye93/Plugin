...
package object Options {
  ...
  var publishImages : Boolean = true
  ...
  def processOptions(options: List[String], error: String => Unit): Unit = {
    ...
    options.foreach {
      ...
      case "publishImages=false" => publishImages = false
  ...
  def checkConstraints(logger : Logger) : Unit = {
    ...
    if(dockerHost.nonEmpty){
      logger.warning(s"You specified a different docker registry. Make sure this registry contains all necessary images, especially the jre base image pulled.")
    }
    if(saveImages)
