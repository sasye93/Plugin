...
import loci.container.build.types.{ModuleConfig, ServiceConfig}
...
class ContainerEntryPoint(from : SimplifiedContainerEntryPoint, mod : Option[ContainerModule])(implicit val plugin : Containerize) extends SimplifiedContainerEntryPoint(
  ...
  lazy val config : ServiceConfig = new ServiceConfig(configPath, if(mod.isDefined) Some(mod.get.config) else None)(io, plugin)
}
